...
public class OrderServiceImpl implements OrderService {
        ...
        if (!CollectionUtils.isEmpty(orders)) {
            ...
            if (StringUtils.isEmpty(errorOrderNos)) {
                ...
                if (orderMapper.closeOrder(Arrays.asList(ids), MallOrderStatusEnum.ORDER_CLOSED_BY_JUDGE.getOrderStatus()) > 0) {
                    // 恢复库存
                    recoverStoreNum(Arrays.asList(ids));
                    return ServiceResultEnum.SUCCESS.getResult();
                } else {
                    return ServiceResultEnum.DB_ERROR.getResult();
                }
    ...
    public String saveOrder(UserVO user, List<ShoppingCartItemVO> myShoppingCartItems) {
        ...
        if (!CollectionUtils.isEmpty(itemIdList) && !CollectionUtils.isEmpty(goodsIds) && !CollectionUtils.isEmpty(goodsList)) {
            if (shoppingCartItemMapper.deleteBatch(itemIdList) > 0) {
                ...
                int updateStockNumResult = goodsMapper.minusStockNum(stockNumDTOS);
    ...
    public String cancelOrder(String orderNo, Long userId) {
        ...
        if (newBeeMallOrder != null) {
            if (orderMapper.closeOrder(Collections.singletonList(newBeeMallOrder.getOrderId()), MallOrderStatusEnum.ORDER_CLOSED_BY_MALLUSER.getOrderStatus()) > 0) {
                // 恢复库存
                recoverStoreNum(Collections.singletonList(newBeeMallOrder.getOrderId()));
                return ServiceResultEnum.SUCCESS.getResult();
            } else {
                return ServiceResultEnum.DB_ERROR.getResult();
            }
    ...
    private void recoverStoreNum(List<Long> orderIdList) {
        // 恢复商品个数
        List<OrderItem> orderItems = orderItemMapper.selectByOrderIds(orderIdList);
        List<StockNumDTO> updateList = new ArrayList<>();
        for (OrderItem orderItem: orderItems) {
            StockNumDTO stockNum = new StockNumDTO();
            stockNum.setGoodsId(orderItem.getGoodsId());
            stockNum.setGoodsCount(orderItem.getGoodsCount());
            updateList.add(stockNum);
        }
        if (!CollectionUtils.isEmpty(updateList)) {
            goodsMapper.plusStockNum(updateList);
        }
    }
}
